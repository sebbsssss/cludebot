...
async function runSchema(url: string, key: string): Promise<{ tables: number; errors: string[] }> {
  ...
  // Try monorepo path first, then npm package path (node_modules/clude/)
  const monorepoPath = path.resolve(__dirname, '..', '..', '..', '..', 'packages', 'database', 'supabase-schema.sql');
  const legacyPath = path.resolve(__dirname, '..', '..', 'supabase-schema.sql');
  let sql: string;
  try {
    sql = fs.readFileSync(monorepoPath, 'utf-8');
  } catch {
    try {
      sql = fs.readFileSync(legacyPath, 'utf-8');
    } catch {
...
function generateEnvFile(config: InitConfig): string {
  let env = `# Generated by clude init\n\n`;
...
export async function runInit(): Promise<void> {
  ...
  if (config.mode === 'hosted') {
    ...
    if (config.supabaseUrl) {
      ...
      printInfo('Schema: paste node_modules/clude/supabase-schema.sql into SQL Editor');
    }
