...
import { broadcastSentimentDigest, broadcastToChannel, type SentimentDigest } from '@clude/brain/services/telegram.service';
...
async function postAITrendsTake(): Promise<void> {
  ...
  try {
    const { searchTokenMentions: search } = await import('@clude/shared/core/x-client');
