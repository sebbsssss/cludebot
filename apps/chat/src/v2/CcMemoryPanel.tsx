...
export function CcMemoryPanel({
  ...
  recalled: V2Memory[];
  other: V2Memory[];
  totals: { stored: number; savedTokToday: number; halluRate: number };
  onClose: () => void;
  ...
  return (
    <aside className="cc-mempanel" role="dialog" aria-label="Memory inspector">
      ...
      <div className="cc-mempanel__foot">
        ...
        <div className="cc-mempanel__stat">
          <span className="cc-mempanel__stat__k">Hallucination rate</span>
          <span className="cc-mempanel__stat__v">{totals.halluRate.toFixed(2)}%</span>
        </div>
      </div>
    </aside>
  );
