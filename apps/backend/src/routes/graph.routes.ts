...
export function graphRoutes(): Router {
  ...
  router.post('/recall', async (req: Request, res: Response) => {
    try {
      ...
      const { withOwnerWallet } = await import('@clude/core/core/owner-context');
      const { recallMemories } = await import('@clude/core/memory');
